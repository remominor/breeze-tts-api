...
def _get_dtype(model: torch.nn.Module) -> torch.dtype:
    """Choose the model compute dtype, ignoring FP32 quantization metadata."""
    ...
    for parameter in model.parameters():
        ...
        fallback = fallback or parameter.dtype
        ...
        try:
            ...
            for step_idx in range(self.config.max_new_tokens):
                ...
                if prefill_len + step_idx >= self.config.max_seq_len - 1:
                    logger.warning(
                        "Breeze generation stopped at the context limit: "
                        "request_id=%s prefill_tokens=%d generated_tokens=%d "
                        "max_seq_len=%d",
                        request_id,
                        prefill_len,
                        step_idx,
                ...
                if chunk_ready or reached_limit:
                    ...
                    if reached_limit:
                        logger.warning(
                            "Breeze generation stopped at max_new_tokens: "
                            "request_id=%s max_new_tokens=%d",
                            request_id,
                            self.config.max_new_tokens,
                        )
                        break
