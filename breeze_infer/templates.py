...
def _resolve_segment_audio_codes(
    ...
    if segment.get("audio_codes") is not None:
        return torch.as_tensor(segment["audio_codes"], dtype=torch.int16).cpu().contiguous()
    ...
    for request in requests:
        ...
        for field in template.required_fields:
            if field == "ref_audio_path":
                present = request.get("ref_audio_path") or request.get("ref_audio_codes") is not None
            else:
                present = request.get(field)
            if not present:
